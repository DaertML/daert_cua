...
def llamacpp_generate(urlllm, prompt, temp=0.6):
    ...
    data = {"prompt": prompt, "temperature": temp, "n_predict":10}
...
If it is a button, choose the <CLICK, ID> action. If it is a text field, provide the <FILL, DATA, ID>; giving in "DATA" the contents to write in the field.
...
Action: 
    ...
    if "CLICK" in res:
        id = res.split(",")[-1].strip().replace(">","")
        print("ID", id)
        coord = label_coordinates[id]
        print("COORD", coord)
