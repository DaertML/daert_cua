...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    ...
    if model_name == "blip2":
        ...
        if device == 'cpu':
            ...
            model_name_or_path, device_map=None, torch_dtype=torch.float32, trust_remote_code=True
        ) 
        else:
            model = Blip2ForConditionalGeneration.from_pretrained(
            model_name_or_path, device_map=None, torch_dtype=torch.float16, trust_remote_code=True
        ).to(device)
