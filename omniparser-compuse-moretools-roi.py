...
# --- Pydantic Action Models (omitted for brevity) ---
...
ActionType = Union[MouseClick, ScrollAction, FillAction, DoneAction, GiveResults, RunCommand]
...
class ComputerAgent:
    def __init__(self, screenshot_roi: Optional[tuple] = None):
        ...
        # NOTE: Failsafe is good, but can interfere with the selection process if the user moves the mouse quickly to a corner during setup.
        pyautogui.FAILSAFE = False # Temporarily disable failsafe to avoid issues during setup. You might want to re-enable it later.
        ...
        print(f"Agent initialized with ROI: {self.screenshot_roi}")
    ...
    def parse_action(self, response: str, label_coordinates: dict) -> ActionType:
        """Parse and validate LLM response using Pydantic, adjusting coordinates for ROI"""
        try:
            ...
            if action_type in ["fill", "click", "right_click", "double_click"]:
                ...
                if element_id in label_coordinates:
                    # Adjust coordinates from local (ROI-based) to global (screen-based)
                    local_coords = label_coordinates[element_id]
                    global_coords = np.array([
                        local_coords[0] + self.offset_x,
                        local_coords[1] + self.offset_y
                    ])
                    json_data["coordinates"] = global_coords
                    print(f"Adjusted ID {element_id} coords: Local {local_coords} -> Global {global_coords}")
                else:
                    raise ValueError(f"ID {element_id} not found in available coordinates.")

            # Try all possible action types
            for model in [MouseClick, ScrollAction, FillAction, DoneAction, GiveResults, RunCommand]:
                try:
    ...
    def run_task(self, task: str):
        ...
        # Ensure failsafe is re-enabled for task execution safety
        ...
        while self.current_step < self.max_steps:
            ...
            # Capture screenshot of the ROI (or full screen if ROI is None)
            ...
            # The coordinates returned by get_som_labeled_img will now be relative to the ROI (the cropped image)
            ...
            # played a whole game of videopoker and held the king
            ...
            # Now, the 'action' object is already validated and has global coordinates
            success = self.execute_action(action, label_coordinates) 
            if not success:
...
# --- MODIFIED ROI SELECTION FUNCTION ---
# --- MODIFIED ROI SELECTION FUNCTION ---
def get_user_roi() -> Optional[tuple]:
    """Prompts the user to select a region of interest using two clicks (Top-Left and Bottom-Right) 
    by waiting for input confirmation before recording the mouse position.
    """
