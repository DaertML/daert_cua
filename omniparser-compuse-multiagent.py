...
class EvalAgent:
    ...
    def eval_step(self, task: str, result: str, ui_elems):
        """Perform the evaluation of the task given the obtained result"""
        
        # Generate prompt for LLM
        prompt = f"""
        You are an AI assistant that evaluates the completion of a task, given the result by an agent. 
        Current task: {task}
        Available UI elements: {ui_elems}
        Obtained result: {result}
                        
        Provide feedback to the model, on wether the task has been successfully completed or not.
        Consider the available UI elements, and if you feel like they are of use for the task to be accomplished.

        In case the task has been completed, suggest it to consider it as "done".
        In case you think the model is in the wrong direction, suggest the next step to perform; the model
        you are evaluating has access to tools to control a computer.
        In case you think the model is in the right direction, but is missing some steps, suggest what to
        do next.
        Also, if you detect that the model is stuck in a loop trying the same actions without an improvement,
        suggest to try something different.
...
class ComputerAgent:
    ...
    def parse_action(self, response: str, label_coordinates: dict) -> ActionType:
        ...
        try:
            ...
            #if action_type in ["give_results"]:
            #    return GiveResults(**json_data)
    ...
    def run_task(self, task: str):
        ...
        while self.current_step < self.max_steps:
            response, ui_elems = self.run_step(task,suggestion,past_actions)
            past_actions.append(response)
            suggestion = self.evalagent.eval_step(task, response, ui_elems)
            
        print("Maximum steps reached. Task incomplete.")
        return False

    def run_step(self, task, suggestion, past_actions):
        self.current_step += 1
        ...
        Suggestion from supervisor: {suggestion}
        ...
        return response, ", ".join(parsed_content_list)
