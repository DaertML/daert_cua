...
# --- Pydantic Action Models (Coordinate-based) ---

class MouseClick(BaseModel):
    action: Literal["click", "right_click", "double_click"]
    coordinates: list = Field(..., description="List of [x, y] coordinates (local to ROI)") 
...
class FillAction(BaseModel):
    action: Literal["fill"]
    coordinates: list = Field(..., description="List of [x, y] coordinates (local to ROI) for text field")
...
class ComputerAgent:
    ...
    def parse_action(self, response: str) -> ActionType:
        ...
        try:
            ...
            if action_type in ["fill", "click", "right_click", "double_click"]:
                ...
                # Coordinates from LLM are assumed to be RELATIVE to the cropped image (ROI)
            ...
            # Try all possible action types (including RunCommand)
    ...
    def run_task(self, task: str):
        ...
        while self.current_step < self.max_steps:
            ...
            # The VLM call is embedded here via get_som_labeled_img
            ...
            # 3. Generate prompt for LLM, including VLM/OCR results
            ...
            Important: 
            - The screenshot provided to you is cropped. All coordinates [x, y] in your output MUST be relative to the top-left corner of the image you see (i.e., local coordinates starting at [0, 0]).
            ...
            try:
                ...
                # Post-processing for clean JSON extraction
            ...
            # 5. Process and execute action
            action = self.parse_action(response) # parse_action handles coordinate transformation
            if not action:
            ...
            # The coordinates in 'action' are now global/absolute, ready for execution
